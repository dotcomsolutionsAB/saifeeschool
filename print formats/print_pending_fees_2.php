...
$due_from       = $_REQUEST['due_from'];
$due_to         = $_REQUEST['due_to'];
...
if($cg_id != 'all')
{
    ...
    $numItems = count($cg_id_array);
    $i = 0;

    foreach ($cg_id_array as $cg_id) {
        $sql = "SELECT * FROM class_group WHERE cg_id = '$cg_id'";
        $query = $db->query($sql);
        ...
        $sql = "SELECT * FROM `student` WHERE `cg_id` = '$cg_id' AND `st_on_roll` = '1' ORDER BY `st_roll_no`";
        $query = $db->query($sql);
        while($row = $query->fetch_assoc())
        {
            ...
            $st_id = $row['st_id'];
            $student_name = $row['st_first_name'].' '.$row['st_last_name'];
            $roll_no = $row['st_roll_no'];

            $flag = 0;
            $final_total_amount = 0;

            $sql_temp = "SELECT * FROM `fee` WHERE `st_id` = '$st_id' AND `fp_id` LIKE '%$fp_id%' AND `ay_id` = '$ay_id' $filter_fee_status AND `f_active` = '1' $filter_type $date_search ORDER BY `fpp_due_date` ASC ";
...
{   
    ...
    $sql_temp = "SELECT * FROM `fee` WHERE `ay_id` = '6' $filter_fee_status AND `f_active` = '1'  $filter_type ORDER BY `cg_id`,`st_roll_no`";
    $query_temp = $db->query($sql_temp);
    while($row_temp = $query_temp->fetch_assoc())
    {
