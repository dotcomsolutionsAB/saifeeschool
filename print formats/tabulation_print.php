...
$allowed_classes = [516,517,518,519,520,521,522,523,524,525,526,534,535,536,537,538,539,540,541,542,543,544];
// Class XI & XII
$higher_secondary = array(505,506,507,508,509,510,511,512,513,514,515,516,517,518,519,520,521,522,523,524,525,526,529,530,532,533,534,535,536,537,538,539,540,541,542,543,544,545,546,547,548,549,550,551,552,553,554,555);
$class_11_12 = array(529,530,532,533,534,535,536,537,538,539,540,541,542,543,544,545,546,547,548,549,550,551,552,553,554,555);
$class_9_10 = array(505,506,507,508,509,510,511,512,513,514,515,516,517,518,519,520,521,522,523,524,525,526);

// Financial Year 2024-25
$pre_primary = array(463,464,465,467,468,473,527);
...
while($row_temp = $query_temp->fetch_assoc()){
    ...
    if(in_array($marks_class, $pre_primary)){
        $col_width = 8;
    }
    else{
        $col_width = 10;
    ...
    $sql_term = "SELECT * FROM `studTerm` WHERE `cg_id` = '$marks_class' AND `term` = '$term'";
    ...
    $pdf->Cell(110,8,$row_term['term_name'].' - '.$row_year['ay_name'],0,1,'L');
    ...
    $sql_subj = "SELECT DISTINCT(subj_name), subj_id, FM.prac 
    FROM studSubjFullMarks AS FM 
    JOIN studSubj AS SB ON FM.subj_id = SB.sub_id 
    WHERE FM.cg_id = '$marks_class' 
    AND FM.term_id = '$term' 
    ORDER BY 
    CASE 
        WHEN SB.category = 'WH' THEN 1
        WHEN SB.category = 'DCC' THEN 2
        WHEN SB.category = 'RC' THEN 3
        WHEN SB.category = 'RD' THEN 4
        WHEN SB.category = 'SP' THEN 5
        WHEN SB.category = 'MT' THEN 6
        WHEN SB.category = 'IL' THEN 7
        WHEN SB.category = 'WR' THEN 8
        ELSE 9  -- For any categories not in the custom order
    END,
    CAST(SB.serial AS UNSIGNED);
    ";
    ...
    while($row_subj = $query_subj->fetch_assoc()){
        ...
        // $pdf->CellFitScale($col_width,40,"",1);
        ...
        if (in_array($marks_class, $higher_secondary) && $subj == 'Geography') {
            $pdf->SetFont('Arial','I',8);
            $pdf->CellFitScale($col_width,40,"",1);
            $pdf->TextWithDirection($x,$y,'History, Geography Average','U');
    ...
    while($row_stud = $query_stud->fetch_assoc()){
        ...
        for($i=0;$i<$len;$i++){
            ...
            $sql_fetch = "SELECT * FROM `studMarks` WHERE `st_roll_no` LIKE '$st_roll_no' AND `subj_id` LIKE '$subj_id' AND `cg_id` LIKE '$marks_class' AND `term` = '$term' LIMIT 1";
            $query_fetch = $db->query($sql_fetch);
            $row_fetch = $query_fetch->fetch_assoc();

            $marks = $row_fetch['marks'];

            if($subj_name[$i] === 'Art' || $subj_name[$i] === 'Arts' || $subj_name[$i] === 'Economic Application' || $subj_name[$i] === 'Home Science ' || strpos($subj_name[$i], 'Biology') !== false){
                if($marks === '0'){
                    $marks = '';
                }
            }
            ...
            if($i==($len-1)){
                $pdf->CellFitScale($col_width,6,$marks,1,0,'C');
            }else{
                $pdf->CellFitScale($col_width,6,$marks,1,0,'C');
            }
            ...
            if($flag[$i] == 1){
                ...
                if($i==($len-1)){
                    $pdf->CellFitScale($col_width,6,$prac,1,0,'C');
                }else{
                    $pdf->CellFitScale($col_width,6,$prac,1,0,'C');
                }
				...
				if($temp_total == 0)
					$temp_total = '';
                if($i==($len-1)){
                    $pdf->CellFitScale($col_width,6,$temp_total,1,0,'C');
                }else{
                    $pdf->CellFitScale($col_width,6,$temp_total,1,0,'C');
                }
            }
            ...
            if (in_array($marks_class, $higher_secondary) && $subj_name[$i] == 'English II') {
                ...
                if($eng_marks_obtained == 0)
					$eng_marks_obtained = '';
                $pdf->CellFitScale($col_width,6,$eng_marks_obtained,1,0,'C');
				
				if($english_avg == 0)
					$english_avg = '';
                if($i==($len-1)){
                    $pdf->CellFitScale($col_width,6,$english_avg,1,0,'C');
                }else{
                    $pdf->CellFitScale($col_width,6,$english_avg,1,0,'C');
                }
            }
            ...
            if ($subj_name[$i] == 'English Literature') {
				...
				if($eng_aggregate == 0)
					$eng_aggregate = '';
                if($i==($len-1)){
                    $pdf->CellFitScale($col_width,6,$eng_aggregate,1,0,'C');
                }else{
                    $pdf->CellFitScale($col_width,6,$eng_aggregate,1,0,'C');
                }
            }

            if($subj_name[$i] == 'History' || $subj_name[$i] == 'Geography'){
                $ss_marks_obtained += intval($marks);
            }

            if (in_array($marks_class, $higher_secondary) && $subj_name[$i] == 'Geography') {
                // if (!in_array($marks_class, $class_11_12)){
                    $average = round($ss_marks_obtained / 2);
                    // $grand_total += intval($average);
                    if($ss_marks_obtained == 0)
                        $average = '';
                    if($i==($len-1)){
                        $pdf->CellFitScale($col_width,6,$average,1,0,'C');
                    }else{
                        $pdf->CellFitScale($col_width,6,$average,1,0,'C');
                    }
                // }
        ...
        if (!in_array($marks_class, $pre_primary)){
            if($grand_total == 0)
                $grand_total = '';
            $pdf->CellFitScale($col_width,6,$grand_total,1,0,'C');
        }

        $sql_fetch = "SELECT * FROM `studAttendance` WHERE `st_roll_no` LIKE '$st_roll_no' AND `term` LIKE '$term' AND `session` = $ay_id";
        $query_fetch = $db->query($sql_fetch);
        $row_fetch = $query_fetch->fetch_assoc();

        $attendance = $row_fetch['attendance'];

        $pdf->CellFitScale($col_width,6,$attendance,1,1,'C');
